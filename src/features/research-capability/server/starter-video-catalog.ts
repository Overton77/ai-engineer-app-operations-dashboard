...
export async function readStarterVideoCatalog(
  ...
  const where: string[] = [catalogChannelScopeSql()];

  applyOptionalCatalogFilters(filters, values, where);
  ...
  const rows = await queryResearchCapability<CatalogQueryRow>(
    `select
       v.video_id,
       v.title,
       v.published_at,
     ...
     ${fromSql}
     ${whereSql}
     ${catalogOrderBySql(filters.sort)}
     ${limitSql}`,
    values,
  );
...
function applyOptionalCatalogFilters(
  filters: StarterVideoCatalogFilters,
  values: unknown[],
  where: string[],
) {
...
function catalogOrderBySql(sort: CatalogSort | undefined): string {
  switch (sort) {
    case "library":
      return "order by v.published_at asc nulls last, v.video_id";
    case "published_asc":
      return "order by v.published_at asc nulls last, v.video_id";
    case "likes_desc":
      return "order by v.like_count desc nulls last, v.published_at desc nulls last, v.video_id";
    case "views_desc":
      return "order by v.view_count desc nulls last, v.published_at desc nulls last, v.video_id";
    case "published_desc":
    default:
      return "order by v.published_at desc nulls last, v.video_id";
  }
}

function mapCatalogRow(row: CatalogQueryRow): StarterVideoCatalogRow {
  return {
